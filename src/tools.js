...
function registerWorkspaceContextTool(context) {
    const tool = {
...
function registerCreateFileTool(context) {
    const tool = {
...
function registerUpdateFileTool(context) {
    const tool = {
        ...
        async invoke(options, token) {
            try {
                ...
                const targetRange = options.input.range
                    ? new vscode.Range(toPosition(options.input.range.start), toPosition(options.input.range.end))
...
function registerDeleteFileTool(context) {
    const tool = {
...
async function gatherWorkspaceContext(input, token) {
...
function resolveWorkspaceUri(candidate) {
    const folders = vscode.workspace.workspaceFolders;
    if (!folders || folders.length === 0) {
        throw new Error(WORKSPACE_ERROR);
    }
    ...
    for (const folder of folders) {
        const joined = vscode.Uri.joinPath(folder.uri, candidate);
        if (isWithin(folder.uri, joined)) {
            return joined;
        }
    }
    throw new Error(`Path "${candidate}" is outside the current workspace.`);
}
async function assertNotExists(uri) {
    try {
        await vscode.workspace.fs.stat(uri);
...
function toGlobPattern(candidate) {
...
async function resolveExistingEntry(candidate, token) {
    const uri = resolveWorkspaceUri(candidate);
    try {
        await vscode.workspace.fs.stat(uri);
        return uri;
...
function positionSchema(description) {
    return {
        type: 'object',
        additionalProperties: false,
        description,
