...
export class PromptService {
  ...
  private composeAgentPrompt(
    ...
    return [
      ...
      "2. When the user references a directory or file name that might be abbreviated or formatted differently (e.g., spaces vs. dashes), list nearby matches with paypilot-workspaceContext and pick the closest existing path before taking action.",
      "3. When you need the full content of a specific file, use paypilot-readFile.",
      "4. Treat the directory listings as authoritative—use them to decide which subdirectory is the best home for new files, and only create a new folder when no close match exists.",
      "5. Use the appropriate create/update/delete/deleteDirectory tool to make each requested change.",
      "6. If you remove directories, confirm the deletion and list notable files that were removed.",
      "7. After finishing tool calls, reply with a concise summary of what changed and include key code snippets if helpful.",
      "⚠️ CRITICAL RULES:",
